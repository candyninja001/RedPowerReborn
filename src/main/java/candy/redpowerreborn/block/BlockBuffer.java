...
public class BlockBuffer extends BlockContainer
{
    ...
    public static void setState(boolean active, World worldIn, BlockPos pos)
    {
        ...
        worldIn.setBlockState(pos, worldIn.getBlockState(pos).getBlock().getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
        worldIn.setBlockState(pos, worldIn.getBlockState(pos).getBlock().getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack)
    {
        ...
        {
            TileEntity tileentity = worldIn.getTileEntity(pos);
            if (tileentity instanceof TileEntityBuffer)
            {
                ((TileEntityBuffer)tileentity).setCustomInventoryName(stack.getDisplayName());
            }
        }
    }
    ...
    {
        return true;//TODO fix ALL comparator input overrides
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        else
        {
            ...
            if (tileentity instanceof TileEntityBuffer)
            {
            	playerIn.openGui(RedPower.instance, GuiHandler.GUI_BUFFER, worldIn, pos.getX(), pos.getY(), pos.getZ());
            }
