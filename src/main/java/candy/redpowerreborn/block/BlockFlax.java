...
public class BlockFlax extends BlockCrops {
	...
	public static final PropertyEnum<BlockDoublePlant.EnumBlockHalf> HALF = PropertyEnum
			.<BlockDoublePlant.EnumBlockHalf> create("half",
	...
	public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source,
			BlockPos pos) {
		return FLAX_AABB[((Integer) state.getValue(this.getAgeProperty()))
				.intValue()];
	}
	...
	protected void checkAndDropBlock(World worldIn, BlockPos pos,
		...
		if (!this.canBlockStay(worldIn, pos, state)) {
			...
			Block block = (Block) (flag ? this : worldIn
					.getBlockState(blockpos).getBlock());
			Block block1 = (Block) (flag ? worldIn.getBlockState(blockpos1)
					.getBlock() : this);
	...
	public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state) {
		if (state.getValue(HALF) == BlockDoublePlant.EnumBlockHalf.UPPER) {
			...
			return iblockstate.getBlock() == this
					&& super.canBlockStay(worldIn, pos, iblockstate);
		}
	}
	...
	public IBlockState withAge(int age) {
		if (age > 7)
			return this
					.getDefaultState()
					.withProperty(AGE, 7)
					.withProperty(this.HALF,
							BlockDoublePlant.EnumBlockHalf.UPPER);
		return this.getDefaultState().withProperty(this.getAgeProperty(),
				Integer.valueOf(age)).withProperty(HALF, BlockDoublePlant.EnumBlockHalf.LOWER);
	}
